...
/**
 * The ViewModel for [MemberListFragment].
 */
@HiltViewModel
class MemberListViewModel @Inject internal constructor(
    ...
    val members: LiveData<List<Member>> = growZone.flatMapLatest { zone ->
        if (zone == NO_GROW_ZONE) {
            memberRepository.getMembers()
        } else {
            memberRepository.getMembersWithGrowZoneNumber(zone)
        }
    }.asLiveData()
